(() => {
  ...
  function renderCategories() { $("[data-category-list]").innerHTML = state.categories.map((c) => `<article class="admin-card"><div><h3>${escape(c.name)}</h3><p>${escape(c.slug)} · ${c.is_active ? "Active" : "Hidden"}</p></div><button data-toggle-category="${c.id}">${c.is_active ? "Hide" : "Activate"}</button></article>`).join(""); }
  ...
  function productForm(product = {}) {
    ...
    return `<h2>${product.id ? "Edit product" : "New product"}</h2><input type="hidden" name="id" value="${product.id || ""}" /><label>Name<input name="name" value="${escape(product.name || "")}" required minlength="2" maxlength="160" /></label><label>Price<input name="price" type="number" min="0" step="0.01" value="${product.price || ""}" required /></label><label>Gender<select name="gender" required>${["Men", "Women", "Unisex"].map((value) => `<option ${value === (product.gender || "Unisex") ? "selected" : ""}>${value}</option>`).join("")}</select></label><label>Availability<select name="availability" required><option value="in" ${availability === "in" ? "selected" : ""}>In Stock</option><option value="low" ${availability === "low" ? "selected" : ""}>Low Stock</option><option value="out" ${availability === "out" ? "selected" : ""}>Out of Stock</option></select></label><label class="wide">Description<textarea name="description" maxlength="5000" required>${escape(product.description || "")}</textarea></label><fieldset class="wide image-source"><legend>Product image</legend><div class="image-source__tabs"><label><input type="radio" name="image_source" value="url" checked /> Image URL</label><label><input type="radio" name="image_source" value="file" /> Upload Image</label></div><label data-image-url>Image URL<input name="external_url" type="url" placeholder="https://example.com/frame.jpg" /></label><label data-image-file hidden>Image file<input name="image_file" type="file" accept="image/jpeg,image/png,image/webp" /></label></fieldset><div class="wide actions"><button class="button button--dark">Save product</button><button class="button" type="button" data-cancel-product>Cancel</button></div>`;
  }
  async function saveProduct(event) { event.preventDefault(); const form = event.target; if (!(form instanceof HTMLFormElement)) return notice("Product form could not be read.", true); const data = new FormData(form); const id = data.get("id"); const name = String(data.get("name") || "").trim(); const price = Number(data.get("price")); const gender = data.get("gender"); const description = String(data.get("description") || "").trim(); const availabilityValue = data.get("availability"); const stockQuantity = availabilityValue === "out" ? 0 : availabilityValue === "low" ? 2 : 20; const urlInput = $("[name='external_url']", form); const fileInput = $("[name='image_file']", form); const externalUrl = urlInput.value.trim(); const file = fileInput.files && fileInput.files.length > 0 ? fileInput.files[0] : null;
    if (name.length < 2 || name.length > 160 || !Number.isFinite(price) || price < 0 || !["Men", "Women", "Unisex"].includes(gender) || !description || description.length > 5000) return notice("Enter a valid name, price, gender, and description.", true);
    const hasFile = fileInput.files && fileInput.files.length > 0;
    const hasUrl = urlInput.value.trim() !== "";
    if (!id && !hasFile && !hasUrl) return notice("Add an external image URL or upload an image file.", true);
    let defaultCategory;
    if (!id) {
      const categoryResult = await client.from("product_categories").select("id").eq("is_active", true).order("sort_order").order("name").limit(1).maybeSingle();
      if (categoryResult.error) return notice(categoryResult.error.message, true);
      if (categoryResult.data?.id) defaultCategory = categoryResult.data.id;
      else {
        const defaultResult = await client.from("product_categories").upsert({ name: "General", slug: "general", is_active: true }, { onConflict: "slug" }).select("id").single();
        if (defaultResult.error || !defaultResult.data?.id) return notice(defaultResult.error?.message || "Could not create the default product category.", true);
        defaultCategory = defaultResult.data.id;
      }
    }
    const payload = { name, price, gender, description, stock_quantity: stockQuantity, status: "published", is_featured: false, category_id: id ? undefined : defaultCategory, sku: id ? undefined : `ADMIN-${crypto.randomUUID()}`, slug: id ? undefined : `admin-${crypto.randomUUID()}` };
